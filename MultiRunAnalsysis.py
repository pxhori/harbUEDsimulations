...
for fname in glob.glob(path):
    if "Run" in fname and ".py" not in fname:
        ...
        csvdata=[]
        csvdata=np.genfromtxt(fname, delimiter=",", skip_header=50)
        multipoints.append(multipoint(runnumber,int(log[runnumber-1][1]*10**6),int(csvdata[12]),int(log[runnumber-1][2]),csvdata[17],10**15*csvdata[5]*2.355/(csvdata[10]*2.998*10**8),csvdata[6]*2.355*1000000))
...
for lss in sorted(lssvalues):
    ...
    for multipoint in multipoints:
        if multipoint.lss==lss:
            plotdata.append([float(multipoint.electronatsample),float(multipoint.emittance),multipoint.length,float(multipoint.fwhmx),multipoint.electronatcathode])
